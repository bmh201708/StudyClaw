...
export function normalizePlannedTasks(input) {
...
export function parseStudyPlan(text) {
    try {
        ...
        if (error instanceof AiServiceError)
            throw error;
        throw new AiServiceError("AI_PARSE_ERROR", `默认 AI 返回内容无法解析为任务计划: ${error instanceof Error ? error.message : "未知错误"}`);
    }
}
