...
function getChatCreditsPer1kTokens(): number {
...
function getWeeklyAllowance(planCode: PlanCode): number {
  return (
    getPlanCatalog().find((plan) => plan.planCode === planCode)?.weeklyCredits ?? DEFAULT_FREE_WEEKLY_CREDITS
  );
}
...
function toUtcFromShanghai(year: number, month: number, date: number, hours = 0): Date {
...
function estimateTokensFromText(...segments: Array<string | undefined>): number {
  const chars = segments.reduce((total, segment) => total + (segment?.length ?? 0), 0);
  return Math.max(1, Math.ceil(chars / 4));
}

function buildInsufficientCredits(
...
export function estimateWorkflowChatReserveCredits(input: {
  ...
  tasks: Array<{ text: string; duration?: string; note?: string; priority?: string }>;
  distractions: string[];
}): number {
  const inputTokens =
    estimateTokensFromText(
      input.goal,
      ...input.messages.map((item) => item.content),
      ...input.tasks.flatMap((task) => [task.text, task.duration, task.note, task.priority]),
      ...input.distractions,
    ) + 1200;
...
export class InsufficientCreditsError extends Error {
  readonly payload: InsufficientCreditsErrorResponse;

  constructor(payload: InsufficientCreditsErrorResponse) {
    super(payload.error);
...
export async function ensureUserSubscription(userId: string, planCode: PlanCode = "free"): Promise<void> {
  ...
  const ledgerCheck = await pool.query<{ exists: boolean }>(
    `
      SELECT EXISTS(
        SELECT 1
        FROM credit_ledger
...
async function refreshWeeklyCreditsIfDue(userId: string): Promise<void> {
  ...
  const now = new Date();
  if (new Date(current.next_credit_reset_at) > now) return;
...
export async function assertHasCredits(userId: string, requiredCredits: number): Promise<SubscriptionSummary> {
  const row = await getSubscriptionRow(userId);
  if (row.current_credits < requiredCredits) {
    throw new InsufficientCreditsError(
      buildInsufficientCredits(row.current_credits, requiredCredits, row.plan_code),
    );
  }
...
export async function consumeCredits(input: {
  ...
  if (row.current_credits < input.requiredCredits) {
    throw new InsufficientCreditsError(
      buildInsufficientCredits(row.current_credits, input.requiredCredits, row.plan_code),
    );
  }
