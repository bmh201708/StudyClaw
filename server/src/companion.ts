...
function computeMidStageScore(
  focusDurationSec: number,
  midStartSec: number,
  tiredThresholdSec: number,
): number {
  if (focusDurationSec <= midStartSec) return MID_START_SCORE;
  if (focusDurationSec >= tiredThresholdSec) return 0;
  const decayProgress =
    (focusDurationSec - midStartSec) / Math.max(tiredThresholdSec - midStartSec, 1);
...
function resolveCameraOnState(
...
): { state: CompanionState; focusScore: number; reasons: string[] } {
  ...
  return {
    ...
    focusScore: round3(Math.max(NORMAL_MIN_SCORE, clamp01(attentionScore))),
    reasons:
      attentionScore >= FOCUS_LOCK_THRESHOLD
        ? ["camera-on baseline normal state"]
        : ["temporary unfocus below sleep threshold; keep normal state"],
  };
}

function resolveCameraOffState(focusDurationSec: number): {
...
function clickActionFor(
  scene: "camera-on" | "camera-off",
  state: CompanionState,
): CompanionEvaluateResponse["clickAction"] {
...
export function evaluateCompanionState(input: CompanionEvaluateBody): CompanionEvaluateResponse {
  ...
  const distressScore = round3(
    clamp01(
      Math.max(
        clamp01(input.metrics?.distressScore ?? 0),
  ...
  const resolved = cameraEnabled
    ? resolveCameraOnState(
        ...
        distressScore,
        tiredThresholdSec,
        Boolean(input.manualPauseActive),
        Boolean(input.distressLocked),
      )
